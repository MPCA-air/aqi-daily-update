  ...
  o3_line_plot <- filter(verify_all, !is.na(`Obs conc`), pollutant == "Ozone") %>%
                  ggplot(aes(x = `Obs conc`, y = `MPCA conc`)) + 
                    ...
                    geom_text_repel(aes(label = Site), size = 1.75) + 
                     scale_color_manual(values = aqi_colors, guide = "none") +
                    ylim(ozone_range) +
                    xlim(ozone_range) +
                    theme_bw() + 
...
verify_o3 <- verify_all %>%
             mutate(across(c(Monitored, MPCA), ~ 
                    paste0("<img src='https://placehold.co/15/", gsub("#", "", conc2color(as.numeric(.x), "ozone")), "/", gsub("#", "", conc2color(as.numeric(.x), "ozone")), ".png' width=11 /> ", .x)))


verify_pm <- verify_all %>%
             mutate(across(c(Monitored, MPCA), ~ 
                    paste0("<img src='https://placehold.co/15/", gsub("#", "", conc2color(as.numeric(.x), "pm25")), "/", gsub("#", "", conc2color(as.numeric(.x), "pm25")), ".png' width=11 /> ", .x)))
